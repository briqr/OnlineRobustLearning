 ...
 * @author Haifeng Li
 */
public class NeuralNetworkRegression implements Classifier<double[]>, SoftClassifier<double[]>, Serializable {
    private static final long serialVersionUID = 1L;
    private static final Logger logger = LoggerFactory.getLogger(NeuralNetworkRegression.class);
    ...
    public static class Trainer extends ClassifierTrainer<double[]> {
        ...
        @Override
        public NeuralNetworkRegression train(double[][] x, int[] y) {
            NeuralNetworkRegression net = new NeuralNetworkRegression(errorFunction, activationFunction, numUnits);
     ...
     * @param numUnits the number of units in each layer.
     */
    public NeuralNetworkRegression(ErrorFunction error, int... numUnits) {
        this(error, natural(error, numUnits[numUnits.length-1]), numUnits);
    }
     ...
     * @param numUnits the number of units in each layer.
     */
    public NeuralNetworkRegression(ErrorFunction error, ActivationFunction activation, int... numUnits) {
        int numLayers = numUnits.length;
        if (numLayers < 2) {
            throw new IllegalArgumentException("Invalid number of layers: " + numLayers);
        }
     ...
     */
    private NeuralNetworkRegression() {
        
    }
    
    @Override
    public NeuralNetworkRegression clone() {
        NeuralNetworkRegression copycat = new NeuralNetworkRegression();
    ...
    private void propagate(Layer lower, Layer upper) {
        for (int i = 0; i < upper.units; i++) {
            ...
            if (upper != outputLayer || activationFunction == ActivationFunction.LOGISTIC_SIGMOID) {
                ...
                //System.out.print("sigmoid value:" + upper.output[i]);
            } else {
    ...
    public int predict(double[] x, double[] y) {
        ...
        if (outputLayer.units == 1) {
            if (outputLayer.output[0] >threshold) {
                return 0;
            } else {
                return 1;
            }
