...
public class ClassifierLearner {
	...
	private NeuralNetwork nnClassifier; 
	...
	public ClassifierLearner(SyncType type) {
		...
		nnClassifier = new NeuralNetwork(ErrorFunction.CROSS_ENTROPY, ActivationFunction.LOGISTIC_SIGMOID, numUnits);
		this.type = type;
	...
	public void processInput(double[] sample, int y) {
		instancesCount.put(type, instancesCount.getOrDefault(type, 0)+1);
		int pred = nnClassifier.predict(sample, y);
